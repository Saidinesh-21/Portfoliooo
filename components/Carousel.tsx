...
const CarouselSlideItem: React.FC<CarouselSlideItemProps> = React.memo(({ item, itemsToShow }) => {
  ...
  const handleMouseEnter = () => {
    hoverTimeoutRef.current = setTimeout(() => {
      setIsHovered(true);
    }, 1000); // 1 second delay before showing description
  };
  ...
  return (
    ...
    >
      <div
        className={`relative w-full h-full p-0.5 sm:p-1 rounded-md transition-transform duration-300 ease-in-out cursor-pointer ${
      ...
      {isHovered && item.description && (
        <div
          className="absolute top-1/2 left-full ml-4 w-48 -translate-y-1/2 bg-[#22223b]/90 text-[#f2e9e4] p-3 rounded-md font-['Roboto Mono'] shadow-lg z-30"
