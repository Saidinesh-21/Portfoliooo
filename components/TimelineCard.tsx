      ...
      >
        <div className="flex items-start mb-4">
          ...
          <div className="flex-grow">
            <p className="text-xs text-[#4a4e69] mb-1 uppercase tracking-wider font-['Roboto Mono']">{date}</p> {/* ultra-violet for date on light bg */}
            <h3 id={`event-title-${id}`} className="text-xl sm:text-2xl font-bold font-heading text-[#22223b] mb-1">{title}</h3> {/* space-cadet for title on light bg */}
            <div className="flex items-center text-sm text-[#4a4e69] font-['Roboto Mono']"> {/* ultra-violet for category text on light bg */}
        ...
        <p className="text-sm text-[#22223b] font-['Roboto Mono'] leading-relaxed">{description}</p> {/* space-cadet for description on light bg */}
      </div>
    </div>
  );
});
