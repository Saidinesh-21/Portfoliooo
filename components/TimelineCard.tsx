...
const CategoryIconComponent: React.FC<{ category: string, type: MediaType }> = ({ category, type }) => {
  const baseClasses = 'w-5 h-5 mr-2 text-gray-600';
  ...
  ({ event, dotOffsetFromCardEdgePx }) => {
    ...
    return (
      <div id={id} className="relative group">
        <div
          className="absolute w-4 h-4 rounded-full bg-gray-400 border-2 border-[#22223b] z-10"
          style={{
        ...
        <div className="bg-white rounded-xl shadow-xl p-5 sm:p-6 transition-all duration-300 ease-in-out group-hover:shadow-gray-500 group-hover:scale-105">
          <div className="flex items-start mb-4">
            ...
            <div className="flex-grow">
              <p className="text-xs text-gray-700 mb-1 uppercase tracking-wider font-['Roboto Mono']">{date}</p>
              <h3
                id={`event-title-${id}`}
                className="text-xl sm:text-2xl font-bold font-heading text-gray-900 mb-1"
              >
                {title}
              </h3>
              <div className="flex items-center text-sm text-gray-700 font-['Roboto Mono']">
          ...
          <p className="text-sm text-gray-800 font-['Roboto Mono'] leading-relaxed">{description}</p>
        </div>
      </div>
    );
  }
