...
const CategoryIconComponent: React.FC<{ category: string; type: MediaType }> = ({ category, type }) => {
  const baseClasses = "w-5 h-5 mr-2 text-[#9a8c98]";
  ...
  if (type === MediaType.GenericVideo || type === MediaType.InstagramReel) return <VideoCameraIcon className={baseClasses} />;
...
const TimelineCard: React.FC<TimelineCardProps> = React.memo(({ event, dotOffsetFromCardEdgePx }) => {
  ...
  return (
    <div id={id} className="relative group">
      <div
        className="absolute w-4 h-4 rounded-full bg-[#9a8c98] border-2 border-[#22223b] z-10"
        style={{
      ...
      <div
        className="bg-[#f0efeb] backdrop-blur-sm rounded-xl shadow-xl p-5 sm:p-6 transform transition-all duration-300 ease-in-out group-hover:shadow-[#9a8c98]/30 group-hover:scale-[1.02]"
      ...
      >
        <div className="flex items-start mb-4">
          <img
            ...
            className="w-12 h-12 sm:w-16 sm:h-16 rounded-full mr-4 border-2 border-[#9a8c98] object-cover flex-shrink-0 group-hover:scale-110 transition-transform duration-200 ease-in-out"
            loading="lazy"
          />
          <div className="flex-grow">
            <p className="text-xs text-[#4a4e69] mb-1 uppercase tracking-wider font-['Roboto Mono']">{date}</p>
            <h3 id={`event-title-${id}`} className="text-xl sm:text-2xl font-bold font-heading text-[#22223b] mb-1">{title}</h3>
